...
class MemberService {
    ...
    private val reputationRepository: ReputationRepository? = null
    private val eventRepository: ReputationEventRepository? = null
    ...
    private val loginFailService: LoginFailService? = null
    private val auditService: SecurityAuditService? = null
    ...
    fun join(username: String, password: String?, name: String, profileImgUrl: String?): Member {
        ...
        memberRepository.findByNickname(nickname)
            .ifPresent(Consumer { _member: Member? ->
                throw ServiceException("409-1", "%s(은)는 사용중인 닉네임입니다.".formatted(nickname))
            })
        ...
        if (username == "system" || username == "admin") member.setRole(Role.ADMIN)
        else member.setRole(Role.USER)

        // (임시) 최초 신용도 50.0
        val reputation = Reputation(member, 50.0)
        reputationRepository!!.save<Reputation?>(reputation)
    ...
    fun modifyOrJoin(username: String, password: String?, nickname: String, profileImgUrl: String?): RsData<Member?> {
        ...
        if (member == null) {
            ...
            return RsData<Member?>("201-1", "회원가입이 완료되었습니다.", member)
        }

        modify(member, nickname, profileImgUrl)

        return RsData<Member?>("200-1", "회원 정보가 수정되었습니다.", member)
    }
    ...
    fun login(member: Member, password: String?) {
        if (member.getStatus() == MemberStatus.BANNED) throw ServiceException("403-3", "계정이 영구 정지되었습니다. 관리자에게 문의해주세요.")

        if (member.getStatus() == MemberStatus.WITHDRAWN) throw ServiceException("403-4", "탈퇴한 계정입니다.")
    ...
    fun withdraw(actor: Member) {
        val member = memberRepository!!.findById(actor.getId())
            .orElseThrow<ServiceException?>(Supplier { ServiceException("404-1", "존재하지 않는 회원입니다.") })
    ...
    @Transactional
    fun modifyNickname(member: Member, nickname: String?) {
    ...
    fun decreaseByNofiy(target: Member, reporter: Member) {
        ...
        // 신고는 하루에 3번만 가능
        if (count >= 3) {
            throw ServiceException("400-6", "신고는 하루에 3번만 가능합니다.")
        }
        ...
        if (exists) {
            throw ServiceException("400-6", "이미 신고한 회원입니다.")
        }

        val report = Report(target, reporter)
        reportRepository.save<Report?>(report)


        val reputation = reputationRepository!!.findById(targetId).get()

        // 이미 정지/탈퇴/영구정지 상태면 신고 누적 X
        if (target.getStatus() == MemberStatus.SUSPENDED || target.getStatus() == MemberStatus.BANNED) throw ServiceException(
            "400-2",
            "해당 회원은 정지된 회원입니다."
        )

        if (target.getStatus() == MemberStatus.WITHDRAWN) throw ServiceException("400-3", "해당 회원은 탈퇴한 회원입니다.")
    ...
    fun decreaseByCancel(auctionId: Int, actorId: Int) {
        ...
        if (auction.bidCount > 0) {
            val reputation = reputationRepository!!.findById(seller.getId()).get()
            ...
            eventRepository!!.save<ReputationEvent?>(event)
        }
    }
    ...
    fun increaseByDeal(dealId: Int) {
        ...
        val seller = memberRepository!!.findById(auction.seller.getId()).get()
    ...
    fun passwordValidation(username: String?, password: String?) {
        ...
        if (username != null && !username.isEmpty() &&
            password.lowercase(Locale.getDefault()).contains(username.lowercase(Locale.getDefault()))
        ) {
    ...
    private fun hasConsecutiveChars(password: String): Boolean {
        ...
        // 연속된 숫자 체크
        val consecutiveNumbers = arrayOf<String?>(
            "012", "123", "234", "345", "456", "567", "678", "789", "890"
        )
        for (seq in consecutiveNumbers) {
    ...
    fun modifyProfile(memberId: Int, profileImg: MultipartFile) {
        if (profileImg == null || profileImg.isEmpty()) {
            throw ServiceException("400-1", "업로드할 이미지 파일이 없습니다.")
        }
        ...
        imageRepository!!.save<Image?>(Image(imageUrl))
